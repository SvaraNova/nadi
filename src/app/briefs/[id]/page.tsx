    ...
    <AppShell dataMode={brief.dataMode}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
              ...
              <span className="badge badge-snapshot">Version {brief.currentVersion}.0</span>
              ...
              {citationsValid && (
                <span className="badge badge-opportunity" style={{ display: "inline-flex", alignItems: "center", gap: "4px" }}>
                  <IconCheck size={12} /> {brief.evidenceRegister.length} Citations Verified
                </span>
              )}
            </div>
            <h1>Executive Decision Brief</h1>
            <p className="page-subtitle">
              Cohort: <strong>{brief.cohortId}</strong> · Reference: <code>{brief.id}</code>
            </p>
          </div>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button
              ...
              disabled={saving}
              title="Create an immutable, non-overwritable version increment"
            >
              Snapshot Version {brief.currentVersion + 1}.0
            </button>
            ...
            >
              {copiedMd ? (
                ...
                <>
                  <IconCopy size={13} /> Copy .md
                </>
              )}
            </button>

            <a
              href={`/api/briefs/${id}/export`}
              download={`${id}.md`}
              className="btn btn-primary btn-sm"
              title="Deterministic offline markdown export"
              style={{ display: "inline-flex", alignItems: "center", gap: "6px" }}
            >
      ...
      {!citationsValid && (
        <div style={{ padding: "12px 16px", background: "var(--warn-100)", border: "1px solid var(--warn-500)", borderRadius: "var(--radius-md)", color: "var(--warn-900)", marginBottom: "20px", fontSize: "0.875rem", display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <div><strong>Citation Warning:</strong> Missing registered evidence IDs: {missingCitations.join(", ")}.</div>
        </div>
      )}
      ...
      {activeTab === "document" ? (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "24px", alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div className="card" style={{ borderTop: "4px solid var(--primary-800)", background: "linear-gradient(180deg, #ffffff 0%, var(--slate-50) 100%)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", paddingBottom: "12px", borderBottom: "1px solid var(--border-light)", marginBottom: "16px", flexWrap: "wrap", gap: "8px" }}>
                <div>
                  <div style={{ fontSize: "0.6875rem", letterSpacing: "0.08em", fontWeight: 800, textTransform: "uppercase", color: "var(--primary-900)" }}>
                    REPUBLIC OF INDONESIA · CABINET POLICY MEMORANDUM
                  </div>
              ...
              <div style={{ marginBottom: "14px" }}>
                ...
                <input
                  ...
                  placeholder="Decision Brief Title"
                />
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "10px", padding: "10px 12px", background: "var(--white)", borderRadius: "var(--radius-sm)", border: "1px solid var(--border-light)", marginBottom: "14px" }}>
                <div>
                  <div style={{ fontSize: "0.6875rem", color: "var(--slate-500)", textTransform: "uppercase", fontWeight: 700 }}>Target Cohort</div>
                  <div style={{ fontWeight: 700, fontSize: "0.875rem", color: "var(--slate-900)" }}>{brief.cohortId}</div>
                </div>
                <div>
                  <div style={{ fontSize: "0.6875rem", color: "var(--slate-500)", textTransform: "uppercase", fontWeight: 700 }}>Quarter Base</div>
                  <div className="tabular-nums" style={{ fontWeight: 700, fontSize: "0.875rem", color: "var(--slate-900)" }}>{brief.period}</div>
                </div>
                <div>
                  <div style={{ fontSize: "0.6875rem", color: "var(--slate-500)", textTransform: "uppercase", fontWeight: 700 }}>Evidence Scope</div>
                  <div className="tabular-nums" style={{ fontWeight: 700, fontSize: "0.875rem", color: "var(--slate-900)" }}>{brief.evidenceRegister.length} Immutable Citations</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <label htmlFor="status-select" style={{ fontSize: "0.8125rem", fontWeight: 700, color: "var(--slate-700)" }}>
                    Document Classification &amp; Review State:
                  </label>
                  <select
                    id="status-select"
                  ...
                  >
                    <option value="draft">Draft (Analyst Working Paper)</option>
                    <option value="reviewed">Reviewed (Verified by Lead Policy Analyst)</option>
                    <option value="final">Final (Locked for Cabinet Distribution)</option>
                  </select>
                </div>
                <div style={{ fontSize: "0.75rem", color: "var(--slate-500)" }}>
                  Version: <strong className="tabular-nums">v{brief.currentVersion}.0</strong>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              <div className="card-eyebrow">Section 1 · High-Level Assessment</div>
              <h2 style={{ fontSize: "1.1rem", margin: "0 0 10px" }}>Executive Summary</h2>
            ...
            <div className="card" style={{ borderLeft: "4px solid var(--primary-700)" }}>
              <div className="card-eyebrow" style={{ color: "var(--primary-800)" }}>Section 2 · Human Analyst Commentary</div>
              <h2 style={{ fontSize: "1.1rem", margin: "0 0 8px" }}>Policy Implications &amp; Recommendations</h2>
              ...
              <textarea
                ...
                placeholder="Enter policy recommendations for Ministry of Energy, Ministry of Finance, or Bank Indonesia..."
              />
            </div>

            {/* Section 3: Structured Findings */}
            <div className="card">
              <div className="card-eyebrow">Section 3</div>
              <h2 style={{ fontSize: "1.1rem", margin: "0 0 12px" }}>Structured Evidence &amp; Findings</h2>
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                      <span className="badge badge-neutral">
                        Finding 3.{i + 1} · {f.kind.toUpperCase()}
                      </span>
                      <div style={{ display: "flex", gap: "4px" }}>
                        {f.citations.map((c, cIdx) => (
                          <button
                            ...
                            title={`Inspect cited evidence ${c}`}
                          >
                            [E{cIdx + 1}]
                          </button>
                        ))}
            ...
            <div className="card">
              <div className="card-eyebrow">Section 4</div>
              <h2 style={{ fontSize: "1.1rem", margin: "0 0 12px" }}>Counterevidence &amp; Public Indicators</h2>
              <div style={{ marginBottom: "14px", padding: "12px 14px", background: "var(--warn-50)", border: "1px solid var(--warn-200)", borderRadius: "var(--radius-md)" }}>
                <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--warn-900)", textTransform: "uppercase", marginBottom: "4px" }}>
                  Divergent Constituents (Counterevidence)
                </div>
              ...
              <div style={{ padding: "12px 14px", background: "var(--slate-50)", border: "1px solid var(--border-light)", borderRadius: "var(--radius-md)" }}>
                <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--slate-700)", textTransform: "uppercase", marginBottom: "4px" }}>
                  Official BPS Macroeconomic Context (Not Comparable)
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            <div className="card">
              <div className="card-eyebrow">Citation Register</div>
              <h3 style={{ margin: "0 0 6px", fontSize: "0.95rem" }}>
                Cited Evidence ({brief.evidenceRegister.length})
              </h3>
              <p style={{ fontSize: "0.75rem", color: "var(--slate-500)", margin: "0 0 12px" }}>
                Each item represents an immutable observation verified by Method v0.1.
              </p>
                ...
                {brief.evidenceRegister.map((item, idx) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: "0.75rem", color: "var(--slate-600)", marginTop: "4px" }}>
                      Metric: {item.metric} · Result: <strong className="tabular-nums">{item.calculation}</strong>
                    </div>
            ...
            <div className="card">
              <div className="card-eyebrow">Provenance &amp; Lineage</div>
              <table style={{ margin: 0, fontSize: "0.75rem" }}>
                <tbody>
                  <tr>
                    <td style={{ color: "var(--slate-500)", padding: "6px 0" }}>Author:</td>
                    <td style={{ padding: "6px 0" }}><strong>{brief.author}</strong></td>
                  </tr>
                  <tr>
                    <td style={{ color: "var(--slate-500)", padding: "6px 0" }}>Dataset:</td>
                    <td style={{ padding: "6px 0" }}><code style={{ fontSize: "0.6875rem" }}>{brief.datasetId}</code></td>
                  </tr>
                  <tr>
                    <td style={{ color: "var(--slate-500)", padding: "6px 0" }}>Signal Run:</td>
                    <td style={{ padding: "6px 0" }}><code style={{ fontSize: "0.6875rem" }}>{brief.signalRunId}</code></td>
                  </tr>
                  <tr>
                    <td style={{ color: "var(--slate-500)", padding: "6px 0" }}>Created:</td>
                    <td style={{ padding: "6px 0" }} className="tabular-nums">{new Date(brief.createdAt).toLocaleString()}</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="card">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "1px solid var(--border-light)", paddingBottom: "12px" }}>
            <div>
              <div className="card-eyebrow">Deterministic Output</div>
              <h2 style={{ margin: 0, fontSize: "1.1rem" }}>Markdown Source Stream</h2>
            </div>
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              >
                {copiedMd ? (
                  <>
                    <IconCheck size={13} /> Copied to Clipboard
                  </>
                ) : (
                  <>
                    <IconCopy size={13} /> Copy Raw Markdown
                  </>
                )}
              </button>
              <a
