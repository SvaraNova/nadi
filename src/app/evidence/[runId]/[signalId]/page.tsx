...
import { Pool } from "pg";
import { readEvidence, publicSourceUrl } from "../../../../server/repositories/evidence";
...
export default async function PersistedEvidence({ params }: { params: Promise<{ runId: string; signalId: string }> }) {
  ...
  let evidence;
  try { evidence = await readEvidence(pool, runId, signalId); }
  ...
  return <main className="evidence">
    ...
    <section><h2>Public comparison</h2><p><strong>Not comparable</strong></p><p>No reviewed indicator mapping is attached to this run. Corporate revenue cannot be equated to real output or employment. {run.mode === "synthetic" ? "Synthetic values cannot establish an observed public-series comparison." : "A reviewed mapping and compatible periods, definitions and geography are required."}</p></section>
  </main>;
}
